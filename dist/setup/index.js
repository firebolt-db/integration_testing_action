...
class Authenticator {
    ...
    static getAuthEndpoint(apiEndpoint) {
        ...
        if (hostStrings[0] != "api") {
            return new URL((0, util_1.assignProtocol)(apiEndpoint)).toString();
        }
        hostStrings[0] = "id";
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.AccountNotFoundError = exports.DeprecationError = exports.ConnectionError = exports.AccessError = exports.AuthenticationError = exports.ArgumentError = exports.ApiError = exports.INVALID_PARAMETERS = exports.MISSING_ENGINE_ENDPOINT = exports.MISSING_DATABASE = exports.MISSING_CLIENT_SECRET = exports.MISSING_CLIENT_ID = void 0;
exports.MISSING_CLIENT_ID = 404001;
exports.MISSING_CLIENT_SECRET = 404002;
exports.MISSING_DATABASE = 404003;
exports.MISSING_ENGINE_ENDPOINT = 404004;
exports.INVALID_PARAMETERS = 400001;
const errorMessages = {
    [exports.MISSING_CLIENT_ID]: "Password is missing",
    [exports.MISSING_CLIENT_SECRET]: "Username is missing",
    [exports.MISSING_DATABASE]: "Database is missing",
    [exports.MISSING_ENGINE_ENDPOINT]: "At least one should be provided: engineName or engineEndpoint",
    [exports.INVALID_PARAMETERS]: "Parameters should be array"
};
class ApiError extends Error {
    constructor({ message, status, code, raw, url }) {
...
class AccountNotFoundError extends Error {
    constructor({ account_name }) {
        const message = `Account '${account_name}' does not exist ` +
...
exports.assignProtocol = assignProtocol;
const isDataQuery = (query) => {
    if (!query) {
        return false;
    }
...
exports.isDataQuery = isDataQuery;
...
exports.withNullableTypes = withNullableTypes;
const checkArgumentExists = (expression, code) => {
    if (!expression) {
        throw new errors_1.ArgumentError({ code });
    }
};
exports.checkArgumentExists = checkArgumentExists;
const checkArgumentValid = (expression, code) => {
    if (!expression) {
        throw new errors_1.ArgumentError({ code });
    }
};
exports.checkArgumentValid = checkArgumentValid;
const systemInfoString = () => {
    return `NodeSDK/${package_json_1.version} (Node ${process.versions.node}; ${os_1.default.platform()} ${os_1.default.release()} )`;
};
exports.systemInfoString = systemInfoString;
const generateUserAgent = (clients, drivers) => {
    const toConnectorString = function (connector) {
    ...
    const driverString = drivers
        ...
        : "";
    return clientString + (0, exports.systemInfoString)() + driverString;
};
exports.generateUserAgent = generateUserAgent;
const zeroPad = (param, length, direction = "left") => {
...
class Connection {
    ...
    getBaseParameters(executeQueryOptions) {
        ...
        const strSettings = Object.entries(settings !== null && settings !== void 0 ? settings : {}).reduce((acc, [key, value]) => {
            if (value !== undefined) {
    ...
    handleUpdateParametersHeader(headerValue) {
    ...
    splitEndpoint(endpoint) {
        const url = new URL(endpoint.startsWith("http") ? endpoint : `https://${endpoint}`.trim());
        const params = Object.fromEntries(url.searchParams.entries());
        return [url.toString().replace(url.search, ""), params];
    }
    async handleUpdateEndpointHeader(headerValue) {
        var _a, _b;
        const [endpoint, newParams] = this.splitEndpoint(headerValue);
        // Validate account_id if present
        const currentAccountId = (_b = (_a = this.accountInfo) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : (await this.resolveAccountId());
    ...
    async execute(query, executeQueryOptions = {}) {
        ...
        executeQueryOptions.response = {
            ...
            ...((_a = executeQueryOptions.response) !== null && _a !== void 0 ? _a : {})
        };
        const { parameters, namedParameters } = executeQueryOptions;
        let setKey = "", setValue = "", formattedQuery;
        if (queryFormatter.isSetStatement(query)) {
            ...
            formattedQuery = queryFormatter.formatQuery(query, parameters, namedParameters);
        }
...
const api_1 = __nccwpck_require__(4236);
const base_1 = __nccwpck_require__(8509);
...
class ConnectionV2 extends base_1.Connection {
    ...
    async getSystemEngineEndpointAndParameters() {
        ...
        try {
            ...
            if (e instanceof errors_1.ApiError && e.status == 404) {
                throw new errors_1.AccountNotFoundError({ account_name: accountName });
            }
            throw e;
        }
    }
    async isDatabaseAccessible(databaseName) {
    ...
    async getEngineUrl(engineName, databaseName) {
        const query = "SELECT engs.url, dbs.database_name, engs.status " +
        ...
        const { data } = await statement.fetchResult();
        if (data.length == 0) {
            throw new errors_1.ConnectionError({ message: `Engine ${engineName} not found.` });
        }
        const filteredRows = [];
        for (const row of data) {
            const [, engintDbName, ,] = row;
    ...
    async getEngineByNameAndDb(engineName, database) {
        ...
        if (!haveAccess) {
            throw new errors_1.AccessError({
    ...
    async getEngineDatabase(engineName) {
        const query = "SELECT attached_to FROM information_schema.engines " +
    ...
    async resolveAccountInfo() {
        if (this.accountInfo === undefined) {
            ...
            const { id, infraVersion } = await httpClient
                .request("GET", url)
                .ready();
            this.accountInfo = { id, infraVersion: parseInt(infraVersion !== null && infraVersion !== void 0 ? infraVersion : "1") };
        }
        return this.accountInfo;
    }
    async resolveEngineEndpoint() {
        ...
        const [systemUrl, systemParameters] = await this.getSystemEngineEndpointAndParameters();
        this.engineEndpoint = new URL(api_1.QUERY_URL, systemUrl).href;
    ...
    getBaseParameters(executeQueryOptions) {
        var _a, _b;
        if (((_a = this.accountInfo) === null || _a === void 0 ? void 0 : _a.infraVersion) == 1) {
            return {
                account_id: (_b = this.accountInfo) === null || _b === void 0 ? void 0 : _b.id,
...
const connection_v1_1 = __nccwpck_require__(585);
const connection_v2_1 = __nccwpck_require__(7885);
...
exports.FireboltCore = FireboltCore;
//# sourceMappingURL=index.js.map
...
const getContext = (options, dependencies) => {
    var _a, _b;
    const { logger: loggerOptions, client: clientOptions, apiEndpoint = DEFAULT_API_ENDPOINT } = options;
    ...
    const httpClient = ((_b = options.dependencies) === null || _b === void 0 ? void 0 : _b.httpClient) || new DefaultHttpClient(clientOptions);
    const queryFormatter = new formatter_1.QueryFormatter();
    const context = {
        logger,
        httpClient,
        apiEndpoint,
...
exports.Tuple = Tuple;
class PGDate extends Date {
}
exports.PGDate = PGDate;
class TimestampTZ extends Date {
...
class TimestampNTZ extends Date {
}
exports.TimestampNTZ = TimestampNTZ;
class QueryFormatter {
    format(query, params, namedParams) {
        params = [...params];
        // Matches:
        ...
        query = query.replace(tokenizer, (str, param, paramName) => {
            if (param) {
                if (params.length == 0) {
    ...
    escapeArray(param, prefix = "[", suffix = "]") {
        let sql = prefix;
        for (let i = 0; i < param.length; i++) {
            const value = param[i];
            const prefix = i === 0 ? "" : ", ";
    ...
    escapeDate(param) {
        ...
        const yearMonthDay = (0, util_1.zeroPad)(year, 4) + "-" + (0, util_1.zeroPad)(month, 2) + "-" + (0, util_1.zeroPad)(day, 2);
        const time = (0, util_1.zeroPad)(hour, 2) +
            ":" +
            (0, util_1.zeroPad)(minute, 2) +
            ":" +
            (0, util_1.zeroPad)(second, 2) +
            (millisecond > 0 ? "." + (0, util_1.zeroPad)(millisecond, 3) : "");
    ...
    escapeObject(param) {
        ...
        else if (bignumber_js_1.default.isBigNumber(param)) {
            return param.toString();
        }
    ...
    formatQuery(query, parameters, namedParameters) {
        if (parameters || namedParameters) {
            if (parameters) {
                (0, util_1.checkArgumentValid)(Array.isArray(parameters), errors_1.INVALID_PARAMETERS);
            }
            if (namedParameters) {
                (0, util_1.checkArgumentValid)(typeof namedParameters === "object", errors_1.INVALID_PARAMETERS);
            }
            query = this.format(query, parameters !== null && parameters !== void 0 ? parameters : [], namedParameters !== null && namedParameters !== void 0 ? namedParameters : {});
        }
        return query;
    }
    isSetStatement(query) {
        return query.trim().toLowerCase().startsWith(SET_PREFIX);
    }
    trimTrailingSemicolon(text) {
        let end = text.length;
    ...
    splitSetStatement(query) {
        ...
        const key = strippedEquation.slice(0, index), value = strippedEquation.slice(index + 1);
...
const agentkeepalive_1 = __nccwpck_require__(4623);
...
const createSocket = agentkeepalive_1.HttpsAgent.prototype.createSocket;
const agentOptions = {
    keepAlive: true,
    keepAliveMsecs: 1000,
    timeout: 0,
...
agentkeepalive_1.HttpsAgent.prototype.createSocket = function (req, options, cb) {
...
class NodeHttpClient {
    constructor() {
        this.getAgent = (url) => {
            ...
            if (this.agentCache.has(hostname)) {
                return this.agentCache.get(hostname);
            }
            const agent = new agentkeepalive_1.HttpsAgent(agentOptions);
    ...
    request(method, url, options) {
        ...
        const makeRequest = async () => {
            ...
            const withProtocol = (0, util_1.assignProtocol)(url);
            const userAgent = headers["user-agent"] || DEFAULT_USER_AGENT;
            const response = await (0, node_fetch_1.default)(withProtocol, {
                agent,
...
__webpack_unused_export__ = (0, firebolt_1.ResourceClient)({
    logger: node_2.Logger,
    httpClient: node_1.NodeHttpClient
});
...
class Meta {
    constructor(column) {
        ...
        this.type = (0, dataTypes_1.getFireboltType)(type);
        this.name = name;
    }
}
exports.Meta = Meta;
...
class DatabaseService {
    ...
    async getByName(databaseName) {
        const query = "SELECT database_name, description FROM information_schema.databases " +
            `WHERE database_name='${databaseName}'`;
        ...
        return new model_2.DatabaseModel({ name, description }, this.context);
    }
    async getAll() {
        const databases = [];
        const query = "SELECT database_name, description FROM information_schema.databases";
        ...
        for (const row of data) {
            ...
            databases.push(new model_2.DatabaseModel({ name, description }, this.context));
        }
        return databases;
    }
    validateCreateOptions(accountVersion, options) {
    ...
    setDefaultCreateOptions(options) {
    ...
    async create(name, options = {}) {
        ...
        let query = `CREATE DATABASE ${options.fail_if_exists ? "" : "IF NOT EXISTS "} "${name}"`;
...
const errors_1 = __nccwpck_require__(1126);
const index_1 = __nccwpck_require__(9861);
...
class DatabaseModel {
    ...
    async delete() {
        ...
        for (const engine of engines) {
            if (engine.current_status_summary == types_1.EngineStatusSummary.STARTING ||
                engine.current_status_summary == types_1.EngineStatusSummary.STOPPING) {
                throw new Error("Unable to call delete: Engine must not be in starting or stopping state.");
            }
        }
...
exports.DatabaseService = void 0;
const api_1 = __nccwpck_require__(4236);
const model_1 = __nccwpck_require__(8762);
...
class EngineService {
    ...
    async getByName(engineName) {
        const query = "SELECT engine_name, url, status FROM information_schema.engines " +
            `WHERE engine_name='${engineName}'`;
        const statement = await this.context.connection.execute(query);
        const { data } = await statement.fetchResult();
        if (data.length == 0) {
        ...
        const [name, endpoint, status] = data[0];
    ...
    async getByDB(database_name) {
        ...
        const { data } = await statement.fetchResult();
        for (const row of data) {
            const [name, endpoint, summary] = row;
    ...
    validateCreateOptions(accountVersion, options) {
        ...
        if (accountVersion >= 2) {
            ...
            const disallowedOptions = disallowedV2Options.filter(option => options[option] !== undefined);
    ...
    setDefaultCreateOptions(accountVersion, options) {
        if (accountVersion == 1 && options.engine_type == undefined) {
            options.engine_type = types_1.EngineType.GENERAL_PURPOSE;
        }
        if (options.fail_if_exists == undefined) {
            options.fail_if_exists = true;
        }
    ...
    async create(name, options = {}) {
        ...
        let query = `CREATE ENGINE ${fail_if_exists ? "" : "IF NOT EXISTS "} "${name}"`;
        const queryParameters = [];
        const createParameterNames = accountVersion >= 2
            ? this.CREATE_PARAMETER_NAMES_V2
            : this.CREATE_PARAMETER_NAMES;
...
class EngineModel {
    ...
    async delete() {
        if (this.current_status_summary == types_1.EngineStatusSummary.DELETING) {
            return;
        }
    ...
    async refreshStatus() {
        const query = "SELECT status FROM information_schema.engines " +
...
exports.processEngineStatus = processEngineStatus;
var EngineStatusSummary;
(function (EngineStatusSummary) {
    EngineStatusSummary["DELETING"] = "Dropping";
    EngineStatusSummary["REPAIRING"] = "Repairing";
    EngineStatusSummary["RUNNING"] = "Running";
    EngineStatusSummary["STARTING"] = "Starting";
...
var EngineType;
(function (EngineType) {
    EngineType["GENERAL_PURPOSE"] = "GENERAL_PURPOSE";
    EngineType["DATA_ANALYTICS"] = "DATA_ANALYTICS";
})(EngineType = exports.EngineType || (exports.EngineType = {}));
var WarmupMethod;
(function (WarmupMethod) {
    WarmupMethod["MINIMAL"] = "MINIMAL";
    WarmupMethod["PRELOAD_INDEXES"] = "PRELOAD_INDEXES";
    WarmupMethod["PRELOAD_ALL_DATA"] = "PRELOAD_ALL_DATA";
})(WarmupMethod = exports.WarmupMethod || (exports.WarmupMethod = {}));
...
exports.EngineService = void 0;
const api_1 = __nccwpck_require__(4236);
const model_1 = __nccwpck_require__(7110);
const utils_1 = __nccwpck_require__(5897);
const v1_1 = __nccwpck_require__(3728);
class EngineService {
    constructor(context) {
        this.context = context;
    }
...
const v1_2 = __nccwpck_require__(533);
...
const getMappedType = (innerType) => {
    const type = typeMapping[innerType];
    if (type) {
        return type;
    }
...
const DATE_TYPES = (0, util_1.withNullableTypes)([
    "pg_date",
    "pgdate",
    "date",
    "timestamp",
...
exports.FLOAT_TYPES = (0, util_1.withNullableTypes)([
    "float",
    "double",
    "decimal",
    "decimal_ext"
]);
exports.BOOLEAN_TYPES = (0, util_1.withNullableTypes)([
    "boolean"
    //"uint8", uncomment after full boolean support for small values in queries like 'select 1;'
]);
exports.INTEGER_TYPES = (0, util_1.withNullableTypes)(["int", "integer", "long"]);
exports.STRING_TYPES = (0, util_1.withNullableTypes)(["string", "text"]);
exports.BYTEA_TYPES = (0, util_1.withNullableTypes)(["bytea"]);
const getFireboltType = (type) => {
    const key = type.toLowerCase();
    const match = key.match(COMPLEX_TYPE);
    if (match) {
        const [_, outerType, innerType] = match;
        if (innerType.match(COMPLEX_TYPE)) {
            return (0, exports.getFireboltType)(innerType);
        }
...
exports.getFireboltType = getFireboltType;
const getInnerType = (type) => {
    ...
    if (match) {
        ...
        if (innerType.match(COMPLEX_TYPE)) {
            return (0, exports.getInnerType)(innerType);
        }
        const mappedType = getMappedType(innerType);
...
const isByteAType = (type) => {
    return exports.BYTEA_TYPES.indexOf(type) !== -1;
};
exports.isByteAType = isByteAType;
...
const isFloatType = (type) => {
    return exports.FLOAT_TYPES.includes(type) || type.match(/decimal(.+)/i);
};
exports.isFloatType = isFloatType;
const isNumberType = (type) => {
    return exports.INTEGER_TYPES.includes(type) || (0, exports.isFloatType)(type);
};
...
const DATE_FORMAT = /^(\d{4})-(\d{1,2})-(\d{1,2})(?:(?:[ \t]+|T)(\d{1,2}):(\d{1,2})(?::(\d{1,2})(?:\.(\d{1,6}))?)?(?:([+-])(\d{1,2})(?::?(\d{2})(?::?(\d{2}))?)?|Z)?)?$/;
const hydrateDate = (value) => {
    const match = DATE_FORMAT.exec(value);
    if (!match) {
        return null;
    }
    // date
    const year = (0, util_1.zeroPad)(match[1], 4);
    const month = (0, util_1.zeroPad)(match[2], 2);
    const day = (0, util_1.zeroPad)(match[3], 2);
    // time
    const hour = (0, util_1.zeroPad)(match[4] || "00", 2);
    const minute = (0, util_1.zeroPad)(match[5] || "00", 2);
    const second = (0, util_1.zeroPad)(match[6] || "00", 2);
    const msec = (0, util_1.zeroPad)(match[7] || "000", 6, "right");
    // tz
    const sign = match[8] || "-";
    const tzHour = (0, util_1.zeroPad)(match[9] || "00", 2);
    const tzMin = (0, util_1.zeroPad)(match[10] || "00", 2);
    // ignore tzsec
    const tzSec = (0, util_1.zeroPad)(match[11] || "00", 2);
    const date = `${year}-${month}-${day}T${hour}:${minute}:${second}.${msec}${sign}${tzHour}:${tzMin}`;
    return new Date(date);
};
exports.hydrateDate = hydrateDate;
//# sourceMappingURL=hydrateDate.js.map
...
const getHydratedValue = (value, type, executeQueryOptions) => {
    var _a;
    if (Array.isArray(value)) {
        const innerType = (0, dataTypes_1.getInnerType)(type);
        return value.map(element => getHydratedValue(element, innerType, executeQueryOptions));
    }
    if ((0, dataTypes_1.isDateType)(type)) {
        return value ? (0, hydrateDate_1.hydrateDate)(value) : value;
    }
    if ((0, dataTypes_1.isNumberType)(type)) {
        if (infNanValues.includes(value)) {
            return hydrateInfNan(value);
        }
        if (((_a = executeQueryOptions.response) === null || _a === void 0 ? void 0 : _a.bigNumberAsString) &&
            typeof value === "object" &&
    ...
    if ((0, dataTypes_1.isByteAType)(type) && value != null) {
        const valueWithoutPrefix = value.substring(2);
        return Buffer.from(valueWithoutPrefix.toString(), "hex");
    }
    return value;
};
const hydrateRow = (row, meta, executeQueryOptions) => {
    const isArray = Array.isArray(row);
    const hydratedRow = isArray ? [] : {};
    for (const [index, column] of meta.entries()) {
        if (isArray) {
            const key = +index;
            hydratedRow[key] = getHydratedValue(row[key], column.type, executeQueryOptions);
        }
        else {
...
exports.hydrateRow = hydrateRow;
//# sourceMappingURL=hydrateResponse.js.map
...
class Statement {
    constructor(context, { query, request, executeQueryOptions }) {
        ...
        this.rowStream = new rowStream_1.RowStream();
    }
    parseResponse(response) {
        const parsed = json_bigint_1.default.parse(response);
    ...
    handleParseResponse(response) {
        try {
            ...
            const isData = (0, util_1.isDataQuery)(this.query);
    ...
    async streamResult(options) {
        const response = await this.request.ready();
        const jsonStream = new jsonStream_1.JSONStream({
        ...
        this.rowStream.on("metadata", (metadata) => {
            resolveMetadata(metadata);
        });
        this.rowStream.on("statistics", (statistics) => {
        ...
        response.body.on("end", () => {
            try {
                const result = jsonStream.getResult(0);
                const statistics = (0, normalizeResponse_1.getNormalizedStatistics)(result.statistics);
    ...
    async fetchResult() {
        ...
        const response = await this.request.ready();
        const text = await response.text();
        if ((_b = (_a = this.executeQueryOptions) === null || _a === void 0 ? void 0 : _a.settings) === null || _b === void 0 ? void 0 : _b.async_execution) {
            return json_bigint_1.default.parse(text);
        }
        const parsed = this.handleParseResponse(text);
        const normalized = (0, normalizeResponse_1.normalizeResponse)(parsed, this.executeQueryOptions);
...
const normalizeRow = (row, meta, executeQueryOptions) => {
    ...
    for (const index in meta) {
        ...
        normalizedRow[name] = row[+index];
    }
    return normalizedRow;
};
exports.normalizeRow = normalizeRow;
const normalizeColumn = (column) => {
    return new meta_1.Meta(column);
};
exports.normalizeColumn = normalizeColumn;
const getNormalizedMeta = (response) => {
    if (!response.meta) {
        return [];
    }
    return response.meta.map(exports.normalizeColumn);
};
const getNormalizedStatistics = (response) => {
    if (response.statistics) {
        const { elapsed, rows_read, bytes_read, scanned_bytes_cache, scanned_bytes_storage } = response.statistics;
        return {
            duration: elapsed,
...
exports.getNormalizedStatistics = getNormalizedStatistics;
const normalizeResponse = (response, executeQueryOptions) => {
    var _a;
    const { response: { normalizeData = false } = {} } = executeQueryOptions;
    const hydrate = ((_a = executeQueryOptions === null || executeQueryOptions === void 0 ? void 0 : executeQueryOptions.response) === null || _a === void 0 ? void 0 : _a.hydrateRow) || hydrateResponse_1.hydrateRow;
    const meta = getNormalizedMeta(response);
    const statistics = (0, exports.getNormalizedStatistics)(response);
    const data = response.data
        ? response.data.map((row) => {
            const hydratedRow = hydrate(row, meta, executeQueryOptions);
            if (normalizeData) {
                const normalizedRow = (0, exports.normalizeRow)(hydratedRow, meta, executeQueryOptions);
                return normalizedRow;
            }
            return hydratedRow;
        })
...
//# sourceMappingURL=normalizeResponse.js.map
...
class JSONStream {
    constructor({ emitter, options, executeQueryOptions }) {
        var _a;
        this.defaultRowParser = (row, isLastRow) => {
            var _a, _b;
            const normalizeData = (_a = this.executeQueryOptions.response) === null || _a === void 0 ? void 0 : _a.normalizeData;
            const parsed = json_bigint_1.default.parse(row);
            const hydrate = ((_b = this.executeQueryOptions.response) === null || _b === void 0 ? void 0 : _b.hydrateRow) || hydrateResponse_1.hydrateRow;
            const result = this.getResult(0);
            const columns = result.columns;
            const hydratedRow = hydrate(parsed, columns, this.executeQueryOptions);
            if (normalizeData) {
                const normalizedRow = (0, normalizeResponse_1.normalizeRow)(hydratedRow, columns, this.executeQueryOptions);
                return normalizedRow;
            }
            return hydratedRow;
        };
        ...
        this.options = options;
        this.executeQueryOptions = executeQueryOptions;
        this.rowParser = ((_a = this.options) === null || _a === void 0 ? void 0 : _a.rowParser) || this.defaultRowParser;
        this.jsonParser = new parser_1.JSONParser({
            onMetadataParsed: columns => {
                this.emitter.emit("metadata", columns);
            },
            hydrateRow: this.rowParser,
            hydrateColumn: (columnStr) => {
                const column = json_bigint_1.default.parse(columnStr);
                return (0, normalizeResponse_1.normalizeColumn)(column);
            }
        });
    }
    processLine(line) {
...
class JSONParser {
    ...
    handleQueryObject(line) {
        if (line.match(/^},?$/)) {
            const queryStr = this.objBuffer + "}";
            const query = json_bigint_1.default.parse(queryStr);
            this.objBuffer = undefined;
            this.state = "rootKeys";
    ...
    handleStatisticsObject(line) {
        if (line.match(/^},?$/)) {
            const queryStr = this.objBuffer + "}";
            const statistics = json_bigint_1.default.parse(queryStr);
            this.objBuffer = undefined;
            this.state = "rootKeys";
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.RowStream = void 0;
const stream_1 = __nccwpck_require__(2781);
class RowStream extends stream_1.Duplex {
    constructor() {
...
//# sourceMappingURL=rowStream.js.map
...
(function (OutputFormat) {
    ...
    OutputFormat["COMPACT"] = "JSON_Compact";
    OutputFormat["JSON_COMPACT"] = "JSONCompact";
...
const { retryWithBackoff } = __nccwpck_require__(4962);
...
/******/ 	var __webpack_exports__ = __nccwpck_require__(2602);
